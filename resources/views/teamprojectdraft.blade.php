        ...
        <form action="{{route('viewSearch')}}" method="post">
            ...
            <div class="row align-items-center my-3">
                ...
                <div class="col-xl-1 col-lg-2 col-md-2">
                    <button type="submit" class="btn_style">Search</button>
                </div>
            </div>
        </form>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-4 product-image__column">
            <div class="card card--no_border">
                <a href="{{route('viewProduct',$products[$i]->id)}}" ><img src="{{$products[$i]->url}}" class="card__product_image card-img-top" /></a>
                <div class="card-body">
                    <a href="{{route('viewProduct',$products[$i]->id)}}">
                        <h4>{{$products[$i]->name}}</h4>
                    </a>
                    <p><span>{{$products[$i]->price}} </span>– {{$products[$i]->categories}}</p>
                </div>
            </div>
        </div>
